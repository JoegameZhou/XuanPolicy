...
from .pettingzoo_wrappers import PettingZooWrapper
...
from .pettingzoo_wrappers import PETTINGZOO_ENVS
...
def make_envs(env_name: str,
    ...
    def _thunk():
        if env_id in TOY_ENVIRONMENTS:
            ...
            scienario = importlib.import_module('environment.pettingzoo.' + env_name + '.' + env_id)
            env = PettingZooWrapper(scienario.parallel_env(), env_name+'.'+env_id)
        else:

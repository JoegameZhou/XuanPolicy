...
class PPOKL_Agent(Agent):
    def __init__(self,
        ...
        super(PPOCLIP_Agent,self).__init__(envs,policy,memory,learner,writer,device,config.logdir,config.modeldir)
        ...
        for step in tqdm(range(train_steps)):
            ...
            for i in range(self.nenvs):
                if dones[i] == True:
                    ...
                    self.writer.add_scalars("exploration",{"env-%d"%i:infos[i]['exploration']},step)
   ...
   # def evaluate(self,training_steps:int,evaluate_steps:int,evaluate_episodes:int):

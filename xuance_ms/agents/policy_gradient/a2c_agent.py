...
class A2C_Agent(Agent):
    def __init__(self,
        ...
        super(A2C_Agent, self).__init__(envs, policy, memory, learner, writer, config.logdir, config.modeldir)
    ...
    def _action(self, obs):
        ...
        if context._get_mode() == 0:
            return {"state": states[0].asnumpy()}, acts, vs.asnumpy()
        else:
            for key in states.keys():
                states[key] = states[key].asnumpy()
            return states, acts, vs.asnumpy()
    ...
    def test(self, test_episodes=100, load_mode=None):

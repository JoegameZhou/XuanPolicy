...
class QTRAN_Learner(LearnerMAS):
    class PolicyNetWithLossCell(nn.Cell):
        ...
        def construct(self, o, ids, a, a_onehot, agt_mask, act_mask, hidden_mask, y_dqn):
            hidden_n, _, q_eval = self._backbone(o, ids)
            q_joint, v_joint = self._backbone.qtran_net(hidden_n['state'] * hidden_mask,
                                                        a_onehot * act_mask)
            ...
            q_joint_greedy_hat, _ = self._backbone.qtran_net(hidden_n['state'] * hidden_mask,
            ...
            if self.agent == "QTRAN_base":
                ...
                q_joint_hat, _ = self._backbone.qtran_net(hidden_n['state'] * hidden_mask, a_onehot * act_mask)
                ...
                q_joint_hat_counterfactual = self._backbone.qtran_net.counterfactual_values_hat(
                    hidden_n['state'] * hidden_mask, a_onehot * act_mask)
    ...
    def __init__(self,
        ...
        super(QTRAN_Learner, self).__init__(config, policy, optimizer, scheduler, summary_writer, modeldir)
    ...
    def update(self, sample):
        ...
        actions_mask = ops.broadcast_to(agent_mask, (-1, -1, self.dim_act))
        hidden_mask = ops.broadcast_to(agent_mask, (-1, -1, self.policy.representation_info_shape['state'][0]))

        hidden_n_next, q_next_eval = self.policy.target_Q(obs_next.view(batch_size, self.n_agents, -1), IDs)
        if self.args.double_q:
            _, actions_next_greedy, _ = self.policy(obs_next, IDs)
        else:
            actions_next_greedy = q_next_eval.argmax(axis=-1, keepdim=False)
        q_joint_next, _ = self.policy.target_qtran_net(hidden_n_next[0] * hidden_mask,

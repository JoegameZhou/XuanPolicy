...
from common.segtree_tool import SumSegmentTree, MinSegmentTree
...
class PerOffPolicyBuffer(Buffer):
    def __init__(self,
        ...
        self._max_prioprity = np.ones((nenvs))
    ...
    def store(self, obs, acts, rews, terminals, next_obs, rep_info, aux_info):
        ...
        for i in range(self.nenvs):
            self._it_sum[i][self.ptr] = self._max_priority ** self._alpha
            self._it_min[i][self.ptr] = self._max_priority ** self._alpha
    ...
    def sample(self, beta=0.4):
        ...
        for i in range(self.nenvs):
            idxes = self._sample_proportional(self.batchsize/self.nenvs)

            weights_ = []
            p_min = self._it_min.min() / self._it_sum[i].sum()
        ...
        obs_batch = sample_batch(self.observations, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        act_batch = sample_batch(self.actions, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        rew_batch = sample_batch(self.rewards, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        terminal_batch = sample_batch(self.terminals, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        next_batch = sample_batch(self.next_observations, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        rep_batch = sample_batch(self.representation_infos, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))
        aux_batch = sample_batch(self.auxiliary_infos, tuple([env_choices.squeeze(0), step_choices.squeeze(0)]))

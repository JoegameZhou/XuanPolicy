...
class PEROffPolicyBuffer(Buffer):
    ...
    def sample(self, beta=0.4):
        ...
        idxes = self._sample_proportional(self.batchsize)
        ...
        # unpack sample
        state  = encoded_sample[0]
        action = encoded_sample[1]
        reward = encoded_sample[2]
        nstate = encoded_sample[3]
        done   = encoded_sample[4]

        # return tuple(list(encoded_sample) + [weights, idxes])
        return (torch.FloatTensor(state).to(self.device),
                torch.FloatTensor(action).to(self.device),
                torch.FloatTensor(reward).to(self.device),
                torch.FloatTensor(nstate).to(self.device),

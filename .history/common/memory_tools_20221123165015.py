...
class PerOffPolicyBuffer(Buffer):
    ...
    def sample(self, beta):
        ...
        obs_batch = sample_batch(self.observations, tuple([env_choices, step_choices.squeeze(0)]))
        act_batch = sample_batch(self.actions, tuple([env_choices, step_choices.squeeze(0)]))
        rew_batch = sample_batch(self.rewards, tuple([env_choices, step_choices.squeeze(0)]))
        terminal_batch = sample_batch(self.terminals, tuple([env_choices, step_choices.squeeze(0)]))
        next_batch = sample_batch(self.next_observations, tuple([env_choices, step_choices.squeeze(0)]))
        rep_batch = sample_batch(self.representation_infos, tuple([env_choices, step_choices.squeeze(0)]))
        aux_batch = sample_batch(self.auxiliary_infos, tuple([env_choices, step_choices.squeeze(0)]))

...
from xuance_torch.agents.agents_marl import linear_decay_or_increase
...
class MFQ_Agents(MARLAgents):
    ...
    def act(self, obs_n, episode, test_mode, act_mean=None, agent_mask=None, noise=False):
        ...
        act_mean = torch.Tensor(act_mean).unsqueeze(dim=-2).repeat(1, self.n_agents, 1)

        _, greedy_actions, q_output = self.policy(obs_in, act_mean, agents_id)
        n_alive = torch.Tensor(agent_mask).sum(dim=-1).unsqueeze(-1).repeat(1, self.dim_act)
        action_n_mask = torch.Tensor(agent_mask).unsqueeze(-1).repeat(1, 1, self.dim_act)
